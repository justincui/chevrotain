...
export function buildTopProduction(impelText:string, name:string, terminals:ITerminalNameToConstructor):gast.Rule {
    ...
    // TODO: can't remove NAME:"bamba" nested rules
...
export function getDirectlyContainedRanges(y:IRange, prodRanges:IProdRange[]):IProdRange[] {
    return filter(prodRanges, (x:IProdRange) => {
        let isXDescendantOfY = y.strictlyContainsRange(x.range)
        let xDoesNotHaveAnyAncestorWhichIsDecendantOfY = every(prodRanges, (maybeAnotherParent:IProdRange) => {
...
let findClosingCurly:(start:number, text:string) => number = <any>partial(findClosingOffset, "{", "}")

let findClosingParen:(start:number, text:string) => number = <any>partial(findClosingOffset, "(", ")")
